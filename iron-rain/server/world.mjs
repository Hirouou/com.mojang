...
import { createServerFrontline } from './frontline.mjs';
import { createServerAirWar } from './air-war.mjs';
import { createBaseService } from './base-service.mjs';
import { createServerRecon } from './recon.mjs';
...
export function createWarAuthority({ store, now = Date.now } = {}) {
  ...
  const frontline = createServerFrontline({ battlefield: state.battlefield, theatre });
  const airWar = createServerAirWar({ battlefield: state.battlefield, theatre });
  const baseService = createBaseService({ state, theatre, online, release, event });
  const recon = createServerRecon({ state, theatre, airWar });
  ...
  const crewOutside = m => occupied(m).some(player => player.exterior?.mamuteId === m.id);
  const near = (player, station) => {
    const pose = online.get(player.id)?.pose, def = stationDefinition(station);
    return Boolean(!player.exterior && pose && def && Math.hypot(pose.x - def.x, pose.z - def.z) <= def.radius + .25);
  };
  ...
  function createMamute(player, payload) {
    ...
    const cleanup = baseService.reset(player); if (!cleanup.ok) return cleanup;
    release(player);
  ...
  function apply(player, type, p = {}) {
    ...
    if (type === 'respawn') {
      ...
      const crew = Object.values(state.players).filter(member => member.mamuteId === m.id);
      // Check every refund before changing any player/vehicle state.
      if (crew.some(member => member.exterior?.carry && !theatre.logistics.getNode(member.exterior.carry.sourceId)?.stock)) return fail('ammo-source-unavailable');
      for (const member of crew) {
        baseService.reset(member); release(member);
        const presence = online.get(member.id);
        if (presence) presence.pose = { x: 0, z: 2.4, yaw: 0, pitch: 0 };
      }
    ...
    const serviceResult = baseService.command(player, type, p);
    if (serviceResult) {
      if (serviceResult.ok && type === 'exit-base') { m.drive = { x: 0, y: 0 }; m.driveUntil = 0; m.robot.speed = 0; }
      return serviceResult;
    }
    if (player.exterior) return fail('operator-outside');
    ...
    if (type === 'drive-vector' || type === 'drive-stop') {
      ...
      if ((x || y) && crewOutside(m)) { m.drive = { x: 0, y: 0 }; m.driveUntil = 0; return fail('crew-outside'); }
      m.drive = { x: x/n, y: y/n }; m.driveUntil = state.time + 1.5;
  ...
  function command(player, request) {
    ...
    state.revision++;
    // Persist discrete transactions immediately. Continuous controls checkpoint
    // with the regular world clock, avoiding full-world SQLite writes per drag.
    if (!['drive-vector', 'aim-delta'].includes(request.type)) save();
    return player.receipts[request.id];
  }
  function heartbeat(player, pose, exteriorPose) {
    expirePresence();
    ...
    let safe = null; try { if (pose && !player.exterior) safe = cabinCrewPose(pose); } catch {}
    const current = online.get(player.id) || {};
    online.set(player.id, { ...current, at: now(), ...(safe ? { pose: safe } : {}) });
    if (player.exterior && exteriorPose !== undefined) {
      const result = baseService.heartbeat(player, exteriorPose);
      // Invalid movement receives the last accepted authoritative pose in the
      // snapshot. It must not turn a healthy connection into an offline state.
      if (!result.ok) return { ok: true, exteriorPoseAccepted: false, exteriorReason: result.reason };
    }
    return { ok: true };
  }
  function tick(dt) {
    ...
    state.battlefield.warSimulation.interestPoints = liveMamutes.filter(m => occupied(m).length).flatMap(m => [m.robot,...(m.projectile ? [m.projectile] : [])]);
    frontline.beforeTick();
    updateWar(state.battlefield, dt);
    for (const capture of frontline.afterTick()) event('territory-captured', null, capture);
    airWar.step(dt);
    ...
    for (const m of liveMamutes) {
      const outside = crewOutside(m);
      if (outside) { m.drive = { x: 0, y: 0 }; m.driveUntil = 0; }
      const v = !outside && m.driveUntil > state.time && engineCanDrive(m.engine) ? m.drive : { x: 0, y: 0 };
      ...
      m.hatchOpen = outside || occupied(m).some(p => { const pos = online.get(p.id)?.pose; return !p.exterior && pos && Math.abs(pos.z - 3.67) < 1.85 && Math.abs(pos.x) < 1.05; });
      ...
      const serviceResult = updateEngine(m.engine, dt, { nearEngine, fireDamage: false });
      ...
      // Fire remains a shared repair problem, not recurring damage from one hit.
  ...
  function snapshot(player, after = state.serial, strategicSince = null) {
    expirePresence();
    ...
    const peers = m ? occupied(m).map((p, seat) => ({ id: p.id, seat, exterior: Boolean(p.exterior),
      pose: p.exterior ? null : online.get(p.id)?.pose || { x: 0, z: 2.4, yaw: 0, pitch: 0 }, station: Object.keys(m.stations).find(key => m.stations[key] === p.id) || null })) : [];
    const sectors = m ? state.battlefield.sectors.filter(s => dist(s, m.robot) < 8000 || (m.projectile && dist(s,m.projectile)<3500)) : [];
    const strategicRevision = Math.floor(state.time / 2);
    const strategic = strategicSince === strategicRevision ? {} : { strategic: {hexes:theatre.hexes,territory:[...theatre.territory],logistics:theatre.logistics.snapshot()} };
    return structuredClone({ ...strategic, strategicRevision, ok: true, authority: 'server', theatreId: state.theatreId, time: state.time, revision: state.revision, eventHead: state.serial,
      ...
      sectors, airWar: airWar.snapshot({ team, position: m?.robot }),
      baseService: baseService.snapshot(player), recon: recon.snapshot(validFaction(player.faction) ? team : null),
      support: state.battlefield.warSimulation.support.filter(s => s.team === team || (m && dist(s, m.robot) < 8000)),
      projectiles: Object.values(state.mamutes).filter(v => v.projectile && (v.id===m?.id || (m && (dist(v.projectile,m.robot)<8000 || dist(v.projectile.origin,m.robot)<2400)))).map(v=>({...v.projectile,mamuteId:v.id})),
      tracers: m ? state.battlefield.tracers.filter(t => dist(t, m.robot) < 8000) : [],
      events: state.events.filter(e => e.id > after && (e.mamuteId === m?.id || (e.type === 'explosion' && m && dist(e.payload, m.robot) < 2400))),
    });
  }
  return { state, theatre, register, identify, authenticate, heartbeat, disconnect, command, snapshot, advance, save, damage, baseService, recon };
}
