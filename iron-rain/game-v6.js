...
(() => {
  ...
  function drawShell(){for(const s of state.serverProjectiles||(state.shell?[state.shell]:[])){const p=worldToScreen(s.x,s.y);if(p.x < -40 || p.y < -40 || p.x>SW+40 || p.y>SH+40)continue;const direction=bearingVector(s.bearing||0);ctx.strokeStyle='#ebc780';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(p.x-direction.x*26,p.y-direction.y*26);ctx.lineTo(p.x,p.y);ctx.stroke();ctx.fillStyle='#ffe08a';ctx.beginPath();ctx.arc(p.x,p.y,5,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.font='10px system-ui';ctx.fillText(`ALT ${Math.max(0,Math.round(s.z))} m`,p.x+12,p.y-10);}}
