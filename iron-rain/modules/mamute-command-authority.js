...
export function createMamuteCommandAuthority({
  ...
  function rememberFireShot(key, shell, ammoRemaining, owner) {
    if (!key) return;
    acceptedFireShots.set(key, Object.freeze({ shell: shell ?? null, ammoRemaining, owner: owner || null }));
  ...
  function receive(command) {
    ...
    const ammoRemaining = inventory ? remainingShells(inventory, shell) : undefined;
    if (shotKey) rememberFireShot(shotKey, shell, ammoRemaining, owner);
