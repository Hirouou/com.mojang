...
export function damageEngine(engine, damage) {
  ...
  if (!amount) return { applied: 0, ignited: false, disabled: !engineCanDrive(engine) };
  ...
  return { applied: before - engine.health, ignited: !wasBurning && engine.fire > 0, disabled: !engineCanDrive(engine) };
}
...
export function serviceEngine(engine, type) {
  ...
  if (type === 'extinguisher') {
    ...
    return { kind: 'equipped', message: alreadyEquipped ? 'Extintor já equipado. Aproxime-se do motor.' : 'Extintor equipado. Leve-o até o motor em chamas.' };
  }
  if (type !== 'engine') return { kind: 'unavailable', message: 'Aproxime-se do motor para fazer a manutenção.' };
  if (engine.fire > 0 && !engine.hasExtinguisher) {
    stop(engine);
    return { kind: 'needs-extinguisher', message: 'Incêndio no motor. Pegue o extintor antes de reparar.' };
  }
  if (engine.action) return { kind: 'busy', message: engine.action.type === 'extinguish' ? 'Combatendo o incêndio. Permaneça junto ao motor.' : 'Reparando o motor. Permaneça junto ao motor.' };
  const actionType = engine.fire > 0 ? 'extinguish' : engine.health < 100 ? 'repair' : null;
  if (!actionType) return { kind: 'ready', message: 'Motor em ordem. O Mamute pode avançar.' };
  ...
  return { kind: 'started', action: actionType, message: actionType === 'extinguish' ? 'Extintor acionado. Mantenha-se junto ao motor por 3 segundos.' : 'Reparo iniciado. Mantenha-se junto ao motor por 6 segundos.' };
}
...
export function updateEngine(engine, dt, { nearEngine = false } = {}) {
  ...
  if (action && (!nearEngine || (action.type === 'repair' && engine.fire > 0) || (action.type === 'extinguish' && !engine.hasExtinguisher))) {
    ...
    return { kind: 'interrupted', message: engine.fire > 0 && action.type === 'repair' ? 'Reparo interrompido. Apague o incêndio primeiro.' : 'Manutenção interrompida. Volte ao motor para continuar.' };
  }
  ...
  if (action.elapsed < action.duration) return null;
  if (action.type === 'extinguish') {
    engine.fire = 0; stop(engine);
    return { kind: 'extinguished', message: 'Incêndio controlado. Interaja com o motor para reparar os danos.', canDrive: engineCanDrive(engine) };
  }
  engine.health = 100; stop(engine);
  return { kind: 'repaired', message: 'Motor reparado. Tração do Mamute restabelecida.', canDrive: true };
}
