...
export function spawnHexesForFaction(faction, hexes = createStrategicHexMap()) {
  ...
  const options = hexes
    ...
    .map(hex => ({
      ...
      y: hex.y,
      team,
      distanceToFront: frontDistance(hex),
...
export function installSpawnSelector({ lobby, onChange = () => {} } = {}) {
  ...
  if (!box) {
    ...
    box.innerHTML = `<style>
      ...
      .crew-spawn-list{display:flex;gap:6px;overflow:auto;padding-bottom:3px;scroll-snap-type:x proximity;-webkit-overflow-scrolling:touch}.crew-spawn-list button{flex:0 0 auto;min-width:132px;min-height:46px;padding:7px 9px;text-align:left;scroll-snap-align:start}.crew-spawn-list button.active{border-color:#e4cc82;box-shadow:inset 0 0 0 1px #e4cc82;color:#f4dda0}.crew-spawn-list button b,.crew-spawn-list button small{display:block}.crew-spawn-list button b{font-size:8px}.crew-spawn-list button small{margin-top:4px;font-size:6.5px;color:#9eaa9a}.crew-spawn-list button.frontline b::before{content:'★ ';color:#e4cc82}.crew-spawn-empty{font-size:8px;color:#c09d76;padding:6px 0}
      @media(max-height:430px) and (orientation:landscape){.crew-spawn{margin:5px 0;padding:6px}.crew-spawn-list button{min-height:32px;min-width:112px;padding:4px 6px}.crew-spawn-head{margin-bottom:4px}.crew-spawn-head small{font-size:6px}}
    </style><div class="crew-spawn"><div class="crew-spawn-head"><b>LOCAL DE NASCIMENTO</b><small>100% DOMINADO · MAIS PERTO DO FRONT PRIMEIRO</small></div><div class="crew-spawn-list" data-spawn-list></div></div>`;
