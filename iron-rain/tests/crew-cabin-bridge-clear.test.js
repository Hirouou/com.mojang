...
test('runtime frame failure releases local station before clearing remote presence', () => {
  ...
  const cabin = {
    snapshot: () => ({ position: { x: 0, z: 2.4 }, yaw: 0, pitch: 0 }),
    leaveStation() { calls.push('leave-station'); return true; },
    updateRemoteCrew(remotes, dt) { calls.push(['remotes', remotes, dt]); },
  };
  const runtime = {
    update() { runtimeUpdateCalled = true; throw new Error('transport frame failed'); },
  ...
  assert.equal(runtimeUpdateCalled, true, 'test must exercise the runtime exception path');
