...
assert.match(source, /function emitLocalShot\(payload = liveShotPayload\(\)\)/, 'live fire must capture one explicit shot snapshot before authority submission');
assert.match(source, /runtime\.issueCommand\('fire', payload\)/, 'identity-bearing fire snapshot must pass through command authority');
