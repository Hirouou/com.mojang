...
assert.match(source, /const shot = liveShotPayload\(runtime\);/, 'identity must be captured with the committed shot payload');
assert.match(source, /runtime\.emitEffect\('reload', \{ duration: 2\.8, phase: 'extract', shell: shot\.shell, shotId: shot\.shotId \}\);/, 'reload must carry the same shot identity as fire');
