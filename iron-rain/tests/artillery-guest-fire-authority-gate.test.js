...
test('guest fire is intercepted before local click or keyboard presentation', async () => {
  ...
  assert.match(intercept, /runtime\.issueCommand\('fire', liveShotPayload\(\)\)/, 'the blocked intent must still go through the shared Mamute authority command');
...
test('accepted local guest fire replays once without issuing a second command', async () => {
  ...
  assert.match(emit, /if \(suppressObservedFire\) \{ suppressObservedFire = false; return; \}/, 'the post-commit text observer must not send a duplicate fire command');
});
